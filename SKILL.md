...
**⚠️ 连环断点：你的错误命令恢复后还会再次中断！**
你发的 lua/run 如果本身有 bug → 断点 → 你 `c` 恢复 → 那条错误代码继续执行 → 又报错 → 又断点。**必须多次 `c` 直到游戏恢复正常心跳，然后查日志找到所有错误，修好代码再重试。** 如果反复 `c` 都恢复不了 → `python game_control.py frestart`。
