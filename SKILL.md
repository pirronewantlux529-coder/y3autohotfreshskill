...
判断断点的方法：**心跳停了 = 断点了**（日志中 >15秒没有新的 `[HEARTBEAT]` 或 `AutoPlayer`）

**⚠️ 心跳规则前置条件**：必须先看到第一个心跳才能用心跳判断断点！游戏刚启动/frestart 后还在加载，没有心跳是正常的。**等到日志中出现第一个 `[HEARTBEAT]` 后，心跳规则才生效。**
...
**`run` 命令已内置自动恢复**：心跳停止 → 自动 continue → 提取错误 → 返回 `[RUN-FAIL]`。但如果自动恢复失败或用的是 `lua` 命令，必须手动执行 `python game_control.py c`。

**绝对禁止：心跳停了还继续发命令 — 游戏已冻结，后续命令全部无效！**
